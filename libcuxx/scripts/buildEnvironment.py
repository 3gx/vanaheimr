...
def defineConfigFlags(env):
	...
	configFlags = env['CXXFLAGS'] + " ".join( 
		["%s\"\\\"%s\\\"\"" % x for x in (
			('-Dlibcuxx_CXXFLAGS=', " ".join(flatten(env['CXXFLAGS']))),
			('-DPACKAGE=', 'libcuxx'),
			('-DVERSION=', env['VERSION']),
			('-Dlibcuxx_PREFIX_PATH=', fixPath(env['INSTALL_PATH'])),
			('-Dlibcuxx_LDFLAGS=', fixPath(env['libcuxx_LDFLAGS'])),
			('-L', fixPath(library_path)),
			('-Dlibcuxx_INCLUDE_PATH=', fixPath(include_path)),
			('-Dlibcuxx_LIB_PATH=', fixPath(library_path)),
			('-Dlibcuxx_BIN_PATH=', fixPath(bin_path))	)])

	env.Replace(libcuxx_CONFIG_FLAGS = configFlags)
