...
{
    *parameters = attribute->schedulingParameters;
...
extern void pthread_cleanup_push(void (*function)(void*), void* argument)
{
	PthreadThreadLocalStorage* local = getPthreadLocalStorage();
	
	PthreadCleanupRoutine routine;
	
	routine->function = function;
	routine->argument = argument;
	
	pthreadCleanupStackPush(local->cleanupRoutineStack, routine);
}
...
extern void pthread_cleanup_pop(int execute)
{
	...
	PthreadCleanupRoutine routine = pthreadCleanupStackPop(local->cleanupRoutineStack);
	
	if(execute)
	{
		routine->function(routine->argument);
	}
}
...
{
	pthread_local_storage_t* local = getPthreadLocalStorage();
	
	if(local->shouldCancel)
	{
		pthread_exit(0);
	}
}
